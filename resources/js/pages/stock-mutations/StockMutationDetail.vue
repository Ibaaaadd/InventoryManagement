...
const handleReject = async () => {
    ...
    try {
        ...
        toastError(
            error.response?.data?.message || "Failed to reject mutation",
        );
    } finally {
        submitting.value = false;
    }
...
const downloadDocument = () => {
    window.open(
        `/api/stock-mutations/${mutation.value.id}/attachment/download`,
        "_blank",
    );
};
...
const viewDocument = () => {
    window.open(
        `/api/stock-mutations/${mutation.value.id}/attachment/view`,
        "_blank",
    );
};
...
<template>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            View and manage stock mutation information
                        </p>
                    </div>
                </div>
            </div>
            <BaseBadge
                v-if="mutation"
                :variant="getStatusVariant(mutation.status)"
                size="lg"
            >
        ...
        <template v-else-if="mutation">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">
                            {{ mutation.item_name_snapshot }}
                        </p>
                        <p class="text-xs text-gray-500">
                            SKU: {{ mutation.item_sku_snapshot }}
                        </p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700"
                    ...
                    <div>
                        ...
                        <p
                            ...
                            :class="{
                                'text-green-600': mutation.type === 'in',
                                'text-red-600': mutation.type === 'out',
                            }"
                        >
                            {{ mutation.type === "in" ? "+" : "-"
                            }}{{ mutation.quantity }}
                    ...
                    <div>
                        ...
                        >
                        <p class="mt-1 text-sm text-gray-900">
                            {{ formatDateTime(mutation.transaction_date) }}
                        </p>
                    </div>
                    <div class="md:col-span-2">
                        ...
                        >
                        <p class="mt-1 text-sm text-gray-900">
                            {{ mutation.notes || "No notes provided" }}
                        </p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700"
                ...
                <div class="space-y-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500">
                                Supporting documentation for this mutation
                            </p>
                        </div>
            ...
            <BaseCard
                v-if="mutation.approvals && mutation.approvals.length > 0"
            ...
            >
                <div class="space-y-4">
                    <div
                        v-for="approval in mutation.approvals"
                        :key="approval.id"
            ...
            <BaseCard :padding="true" :shadow="true">
                <AuditTrailPanel
                    auditable-type="App\Models\StockMutation"
                    :auditable-id="mutation.id"
                />
            </BaseCard>
        </template>
        ...
        <BaseModal v-model="showApproveModal" title="Approve Stock Mutation">
            <div class="space-y-4">
                ...
                <div v-if="mutation" class="bg-slate-50 p-3 rounded-lg text-sm">
                    <p>
                        <strong>Item:</strong> {{ mutation.item_name_snapshot }}
                    </p>
                    <p>
                        <strong>Type:</strong>
